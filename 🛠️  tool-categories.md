# 🛠️ Tool Categories
## *Organized by Phase, Purpose, and Complexity*

---

> **Navigation Tip**: *Use Ctrl+F (or Cmd+F) to quickly find specific tools or phases!*

---

## 🗺️ Quick Navigation

<div align="center">

| 🔍 [**Discovery**](#discovery-phase) | 🎨 [**Co-Creation**](#co-creation-phase) | ⚙️ [**Development**](#development-phase) | 🚀 [**Deployment**](#deployment-phase) |
|:---:|:---:|:---:|:---:|
| *Understand users & context* | *Design solutions together* | *Build & test iteratively* | *Launch & sustain adoption* |

</div>

---

## 🔍 Discovery Phase
*"Who are our users and what do they really need?"*

<details>
<summary><strong>🎯 Phase Overview</strong></summary>

**Goal**: Identify core needs, relevant actors, and entry points through participatory, inclusive engagement.

**Key Questions**:
- Who are the end users and what are their roles?
- What problems are they trying to solve?
- What are their current workflows and decision-making processes?
- What constraints do they face (technical, institutional, temporal)?

**Typical Duration**: 2-6 weeks
**Source**: *MSFC Coordination on Solutions Co-Development Toolkit*

</details>

### 🛠️ Available Tools

<table>
<tr>
<th width="25%">Tool</th>
<th width="15%">Difficulty</th>
<th width="15%">Duration</th>
<th width="45%">Best For</th>
</tr>
<tr>
<td>

**🗺️ [Stakeholder Mapping Workshop](https://github.com/your-repo/issues/1)**

</td>
<td>

🌱 Beginner

</td>
<td>

2-3 hours

</td>
<td>

Understanding who's involved, their relationships, and influence levels. Great starting point for any project.

</td>
</tr>
<tr>
<td>

**💬 [Discovery Interview Blueprint](https://github.com/your-repo/issues/2)**

</td>
<td>

📈 Intermediate

</td>
<td>

45-90 min per interview

</td>
<td>

Deep-dive conversations when surveys won't capture the full picture. Excellent for complex workflows.

</td>
</tr>
<tr>
<td>

**📋 [Needs Assessment Survey Design](https://github.com/your-repo/issues/3)**

</td>
<td>

🌱 Beginner

</td>
<td>

1-2 weeks

</td>
<td>

Gathering structured data from larger user groups. Perfect for validating assumptions at scale.

</td>
</tr>
<tr>
<td>

**🔄 [Information Flow Analysis](https://github.com/your-repo/issues/4)**

</td>
<td>

🎯 Advanced

</td>
<td>

3-5 days

</td>
<td>

Tracing how data moves from source to action. Critical for identifying bottlenecks and broken handoffs.

</td>
</tr>
<tr>
<td>

**🎯 [Context Analysis Framework](https://github.com/your-repo/issues/5)**

</td>
<td>

📈 Intermediate

</td>
<td>

1 week

</td>
<td>

Understanding institutional, technical, and cultural constraints that shape solution adoption.

</td>
</tr>
</table>

---

## 🎨 Co-Creation Phase
*"How do we design the right solution together?"*

<details>
<summary><strong>🎯 Phase Overview</strong></summary>

**Goal**: Work directly with stakeholders to co-create feasible, locally owned solutions that fit real-world use contexts.

**Key Activities**:
- Co-design workshops and collaborative planning sessions
- Requirements definition with end users and system operators
- User journey mapping and workflow integration planning
- Output validation and format specification

**Typical Duration**: 3-8 weeks
**Source**: *Solution Co-Development Toolkit Narrative; SERVIR Service Design Tool 2021*

</details>

### 🛠️ Available Tools

<table>
<tr>
<th width="25%">Tool</th>
<th width="15%">Difficulty</th>
<th width="15%">Duration</th>
<th width="45%">Best For</th>
</tr>
<tr>
<td>

**🏗️ [Co-Design Workshop Facilitator Manual](https://github.com/your-repo/issues/6)**

</td>
<td>

📈 Intermediate

</td>
<td>

Half-day workshop

</td>
<td>

Structured sessions to align scientists and users around solution goals. Includes pre/during/post guidance.

</td>
</tr>
<tr>
<td>

**📋 [Requirements Definition Canvas](https://github.com/your-repo/issues/7)**

</td>
<td>

🌱 Beginner

</td>
<td>

2-3 hours

</td>
<td>

Visual template for capturing functional requirements, interface logic, and output formats collaboratively.

</td>
</tr>
<tr>
<td>

**🗺️ [User Journey Mapping Kit](https://github.com/your-repo/issues/8)**

</td>
<td>

📈 Intermediate

</td>
<td>

4-6 hours

</td>
<td>

Mapping how users will interact with solutions from start to finish. Identifies barriers early.

</td>
</tr>
<tr>
<td>

**✅ [Output Validation Checklist](https://github.com/your-repo/issues/9)**

</td>
<td>

🌱 Beginner

</td>
<td>

1-2 hours

</td>
<td>

Systematic verification of key parameters (resolution, frequency, latency, formats) with stakeholders.

</td>
</tr>
<tr>
<td>

**🎯 [Service Concept Development Instructions](https://github.com/your-repo/issues/10)**

</td>
<td>

🎯 Advanced

</td>
<td>

1-2 weeks

</td>
<td>

Creating shared service vision documents that articulate impact pathways and technical specifications.

</td>
</tr>
</table>

---

## ⚙️ Development Phase
*"How do we build and test with users?"*

<details>
<summary><strong>🎯 Phase Overview</strong></summary>

**Goal**: Translate user needs into functional, user-centered technical solutions through iterative testing and refinement.

**Key Activities**:
- Prototype development with continuous stakeholder input
- User testing and iterative design refinement
- Technical validation with end users
- Usability barrier identification and resolution

**Typical Duration**: 8-20 weeks (iterative)
**Source**: *Solution Co-Development Toolkit Narrative; NSITE Solution Project Requirements*

</details>

### 🛠️ Available Tools

<table>
<tr>
<th width="25%">Tool</th>
<th width="15%">Difficulty</th>
<th width="15%">Duration</th>
<th width="45%">Best For</th>
</tr>
<tr>
<td>

**🧪 [User Testing Protocol](https://github.com/your-repo/issues/11)**

</td>
<td>

📈 Intermediate

</td>
<td>

2-3 hours per session

</td>
<td>

Structured approach to testing prototypes with real users. Includes observation templates and analysis frameworks.

</td>
</tr>
<tr>
<td>

**🎨 [Prototype Review Session Instructions](https://github.com/your-repo/issues/12)**

</td>
<td>

🌱 Beginner

</td>
<td>

1-2 hours

</td>
<td>

Collaborative feedback sessions at key development decision points. Great for maintaining alignment.

</td>
</tr>
<tr>
<td>

**📊 [Usability Metrics Dashboard](https://github.com/your-repo/issues/13)**

</td>
<td>

🎯 Advanced

</td>
<td>

Ongoing

</td>
<td>

Tracking task completion rates, error rates, time-to-competency, and satisfaction scores.

</td>
</tr>
<tr>
<td>

**🔄 [Design Iteration Framework](https://github.com/your-repo/issues/14)**

</td>
<td>

📈 Intermediate

</td>
<td>

1-2 weeks per cycle

</td>
<td>

Systematic approach to incorporating user feedback into design improvements. Includes prioritization methods.

</td>
</tr>
<tr>
<td>

**✅ [Technical Validation Checklist](https://github.com/your-repo/issues/15)**

</td>
<td>

🌱 Beginner

</td>
<td>

2-4 hours

</td>
<td>

Ensuring technical specifications match user requirements and real-world constraints.

</td>
</tr>
</table>

---

## 🚀 Deployment Phase
*"How do we ensure successful adoption and sustained use?"*

<details>
<summary><strong>🎯 Phase Overview</strong></summary>

**Goal**: Ensure effective solution adoption through proper training, support, and continued user engagement.

**Key Activities**:
- Soft launch with co-design participants
- Training material development and delivery
- Ongoing support system establishment
- Adoption monitoring and sustainability planning

**Typical Duration**: 4-12 weeks (ongoing support)
**Source**: *NSITE Solution Project Requirements; SERVIR Capacity Building and Training Definition*

</details>

### 🛠️ Available Tools

<table>
<tr>
<th width="25%">Tool</th>
<th width="15%">Difficulty</th>
<th width="15%">Duration</th>
<th width="45%">Best For</th>
</tr>
<tr>
<td>

**🎯 [Soft Launch Strategy Template](https://github.com/your-repo/issues/16)**

</td>
<td>

📈 Intermediate

</td>
<td>

2-4 weeks

</td>
<td>

Limited release to co-design participants for final validation before broader deployment.

</td>
</tr>
<tr>
<td>

**📚 [Training Material Development Kit](https://github.com/your-repo/issues/17)**

</td>
<td>

📈 Intermediate

</td>
<td>

3-6 weeks

</td>
<td>

Creating user documentation, quick-start materials, and training content that matches user skill levels.

</td>
</tr>
<tr>
<td>

**🔧 [Support System Setup Instructions](https://github.com/your-repo/issues/18)**

</td>
<td>

🌱 Beginner

</td>
<td>

1-2 weeks

</td>
<td>

Establishing channels for user questions, issue reporting, and ongoing communication.

</td>
</tr>
<tr>
<td>

**📊 [Adoption Monitoring Framework](https://github.com/your-repo/issues/19)**

</td>
<td>

🎯 Advanced

</td>
<td>

Ongoing

</td>
<td>

Tracking actual usage patterns, gathering impact stories, and measuring solution effectiveness.

</td>
</tr>
<tr>
<td>

**🌱 [Sustainability Planning Workshop](https://github.com/your-repo/issues/20)**

</td>
<td>

📈 Intermediate

</td>
<td>

Half-day workshop

</td>
<td>

Working with users to identify long-term ownership models and maintenance approaches.

</td>
</tr>
</table>

---

## 🎯 Cross-Phase Tools
*"Tools that work throughout multiple phases"*

<table>
<tr>
<th width="25%">Tool</th>
<th width="15%">Phases</th>
<th width="15%">Difficulty</th>
<th width="45%">Best For</th>
</tr>
<tr>
<td>

**🤝 [Stakeholder Engagement Planner](https://github.com/your-repo/issues/21)**

</td>
<td>

All phases

</td>
<td>

📈 Intermediate

</td>
<td>

Scheduling touchpoints, managing communication, and maintaining relationships throughout project lifecycle.

</td>
</tr>
<tr>
<td>

**📝 [Decision Documentation Template](https://github.com/your-repo/issues/22)**

</td>
<td>

All phases

</td>
<td>

🌱 Beginner

</td>
<td>

Recording design decisions, user input, alternatives considered, and rationale for choices.

</td>
</tr>
<tr>
<td>

**📊 [Progress Communication Dashboard](https://github.com/your-repo/issues/23)**

</td>
<td>

Development + Deployment

</td>
<td>

📈 Intermediate

</td>
<td>

Keeping stakeholders informed of progress and demonstrating how their feedback shaped solutions.

</td>
</tr>
</table>

---

## 🔍 Tool Finder by Characteristics

### By Time Investment

<details>
<summary><strong>⚡ Quick Wins (< 2 hours)</strong></summary>

- 📋 [Output Validation Checklist](https://github.com/your-repo/issues/9)
- ✅ [Technical Validation Checklist](https://github.com/your-repo/issues/15)
- 📝 [Decision Documentation Template](https://github.com/your-repo/issues/22)
- 🎨 [Prototype Review Session Instructions](https://github.com/your-repo/issues/12)

</details>

<details>
<summary><strong>⏰ Half-Day Commitment (2-4 hours)</strong></summary>

- 🗺️ [Stakeholder Mapping Workshop](https://github.com/your-repo/issues/1)
- 📋 [Requirements Definition Canvas](https://github.com/your-repo/issues/7)
- 🧪 [User Testing Protocol](https://github.com/your-repo/issues/11)
- 🌱 [Sustainability Planning Workshop](https://github.com/your-repo/issues/20)

</details>

<details>
<summary><strong>📅 Multi-Day Projects (1+ weeks)</strong></summary>

- 📋 [Needs Assessment Survey Design](https://github.com/your-repo/issues/3)
- 🔄 [Information Flow Analysis](https://github.com/your-repo/issues/4)
- 🎯 [Service Concept Development Instructions](https://github.com/your-repo/issues/10)
- 📚 [Training Material Development Kit](https://github.com/your-repo/issues/17)

</details>

### By Team Size

<details>
<summary><strong>👤 Solo/Small Team (1-3 people)</strong></summary>

- 📋 [Needs Assessment Survey Design](https://github.com/your-repo/issues/3)
- 💬 [Discovery Interview Blueprint](https://github.com/your-repo/issues/2)
- 📝 [Decision Documentation Template](https://github.com/your-repo/issues/22)
- ✅ [Technical Validation Checklist](https://github.com/your-repo/issues/15)

</details>

<details>
<summary><strong>👥 Medium Team (4-8 people)</strong></summary>

- 🗺️ [Stakeholder Mapping Workshop](https://github.com/your-repo/issues/1)
- 🏗️ [Co-Design Workshop Facilitator Manual](https://github.com/your-repo/issues/6)
- 🗺️ [User Journey Mapping Kit](https://github.com/your-repo/issues/8)
- 🧪 [User Testing Protocol](https://github.com/your-repo/issues/11)

</details>

<details>
<summary><strong>👫 Large Group (9+ people)</strong></summary>

- 🎯 [Context Analysis Framework](https://github.com/your-repo/issues/5)
- 🎯 [Service Concept Development Instructions](https://github.com/your-repo/issues/10)
- 🌱 [Sustainability Planning Workshop](https://github.com/your-repo/issues/20)

</details>

### By Experience Level

<details>
<summary><strong>🌱 New to Co-Design</strong></summary>

**Start with these beginner-friendly tools:**
- 🗺️ [Stakeholder Mapping Workshop](https://github.com/your-repo/issues/1) - Learn who's involved
- 📋 [Requirements Definition Canvas](https://github.com/your-repo/issues/7) - Capture what users need
- ✅ [Output Validation Checklist](https://github.com/your-repo/issues/9) - Verify you're on track
- 📝 [Decision Documentation Template](https://github.com/your-repo/issues/22) - Keep records

</details>

<details>
<summary><strong>📈 Some Experience</strong></summary>

**Ready for intermediate challenges:**
- 💬 [Discovery Interview Blueprint](https://github.com/your-repo/issues/2)
- 🏗️ [Co-Design Workshop Facilitator Manual](https://github.com/your-repo/issues/6)
- 🧪 [User Testing Protocol](https://github.com/your-repo/issues/11)
- 📚 [Training Material Development Kit](https://github.com/your-repo/issues/17)

</details>

<details>
<summary><strong>🎯 Advanced Practitioner</strong></summary>

**Complex tools for experienced teams:**
- 🔄 [Information Flow Analysis](https://github.com/your-repo/issues/4)
- 🎯 [Service Concept Development Instructions](https://github.com/your-repo/issues/10)
- 📊 [Usability Metrics Dashboard](https://github.com/your-repo/issues/13)
- 📊 [Adoption Monitoring Framework](https://github.com/your-repo/issues/19)

</details>

---

## 📄 Sources and Attribution

Tool categories and descriptions synthesized from:
- **MSFC Coordination on Solutions Co-Development Toolkit** - Phase definitions and workflows
- **NSITE Solution Project Requirements and Expectations** - Tool applications and timing
- **SERVIR Service Planning Toolkit 2021** - Service design methodology
- **Solution Co-Development Toolkit Narrative** - Iterative development approaches

---

<div align="center">

**🎯 Can't find what you need? [Suggest a new tool!](https://github.com/your-repo/issues/new?template=new-tool.md)**

[← Framework Overview](framework-overview.md) | [Next: Implementation Manual →](implementation-manual.md)

</div>
